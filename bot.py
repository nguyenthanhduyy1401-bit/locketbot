...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = "4HicN3RYrtcpUNxJLEIUEtXgNL22"
    if user_id !=6849106749 : return
    ...
    if data.startswith("upg|"):
        ...
        async with queue_lock:
            ...
            for pending in pending_items:
                if pending['uid'] == uid:
                    try:
                        await query.answer("⚠️ This User is already in the queue!", show_alert=True)
                    except:
                        pass
                    return
